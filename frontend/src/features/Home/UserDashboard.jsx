...
const UserList = ({ onEdit }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:4000users'); // Adjust endpoint as needed
  ...
  const handleDelete = async (userId) => {
    try {
      await axios.delete(`http://localhost:4000users/${userId}`);
...
const UserDashboard = () => {
  ...
  const handleCreateUser = async (newUser) => {
    try {
      ...
      const response = await axios.post('http://localhost:4000users', newUser);
  ...
  const handleUpdateUser = async (updatedUser) => {
    try {
      ...
      const response = await axios.put(`http://localhost:4000users/${updatedUser._id}`, updatedUser);
