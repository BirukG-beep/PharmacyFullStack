...
const { medicines, Loading, error } = useListMedicine(token);
...
useEffect(() => {
  ...
  const uniqueGroups = medicines.reduce((acc, medicine) => {
    if (medicine.medicineGroup && !acc.includes(medicine.medicineGroup)) {
      acc.push(medicine.medicineGroup);
    }
    return acc;
  }, []);

  setMedicineGroups(uniqueGroups);
}, [medicines, Loading]);   // ← only re-run when these actually change
